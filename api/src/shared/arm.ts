import { AuthUser, ServerItem } from "./types.js";
...
export async function listServersForUser(_user: AuthUser, armAccessToken?: string | null): Promise<ServerItem[]> {
  if (!armAccessToken) {
    return [
      {
    ...
    subscriptions.map(async (sub) => {
      ...
      const [vms, arcMachines] = await Promise.all([
        armGetAll<{ id: string; name: string; properties?: { storageProfile?: { osDisk?: { osType?: string } } } }>(
          vmUrl, armAccessToken
        ).catch(() => []),
        armGetAll<{ id: string; name: string; properties?: { osName?: string } }>(
          arcUrl, armAccessToken
        ).catch(() => [])
      ]);
